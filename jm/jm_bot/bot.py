...
current_date = datetime.now().strftime('%Y-%m-%d')
log_file = f'log/bot_{current_date}.log'
...
if PDF_ENABLED:
    logger.info("PDF模式已启用，将使用PDF下载方式")
else:
    logger.info("PDF模式未启用，将使用ZIP下载方式")
...
async def download_pdf(jm_id: str, user_id: str) -> Optional[str]:
    ...
    try:
        ...
        try:
            ...
            pdf_dir = os.path.join(os.getcwd(), "jm_bot", "pdf")
            os.makedirs(pdf_dir, exist_ok=True)
            os.chdir(pdf_dir)
            logger.info(f"切换到PDF目录: {pdf_dir}")
        ...
        if os.path.exists(user_download_dir):
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    logger.info(f"已清理用户下载目录: {user_download_dir}")
                    break
                except Exception as e:
                    if attempt < max_retries - 1:
                        logger.warning(f"删除用户下载目录失败，等待1秒后重试: {e}")
                        await asyncio.sleep(1)
                    else:
                        logger.error(f"删除用户下载目录失败: {e}")

        # 清理ZIP文件
        zip_path = os.path.join(os.getcwd(), ZIP_DIR, f"{jm_id}.zip")
        inner_zip_path = os.path.join(os.getcwd(), ZIP_DIR, f"{jm_id}_inner.zip")
        
        for file_path in [zip_path, inner_zip_path]:
            if os.path.exists(file_path):
                max_retries = 10
                for attempt in range(max_retries):
                    try:
                        os.remove(file_path)
                        logger.info(f"已删除ZIP文件: {file_path}")
                        break
                    except Exception as e:
                        if attempt < max_retries - 1:
                            logger.warning(f"删除ZIP文件失败，等待1秒后重试: {e}")
                            await asyncio.sleep(1)
                        else:
                            logger.error(f"删除ZIP文件失败: {e}")

        # 清理PDF文件
        pdf_path = os.path.join(os.getcwd(), "jm_bot", "pdf", f"{jm_id}.pdf")
        if os.path.exists(pdf_path):
            max_retries = 10
            for attempt in range(max_retries):
                try:
                    ...
                    if attempt < max_retries - 1:
                        logger.warning(f"删除PDF文件失败，等待1秒后重试: {e}")
                        await asyncio.sleep(1)
                    else:
                        logger.error(f"删除PDF文件失败: {e}")

        # 清理jm_bot目录下的downloads
        jm_bot_downloads = os.path.join(os.getcwd(), "jm_bot", "downloads", str(user_id))
        if os.path.exists(jm_bot_downloads):
            max_retries = 10
            for attempt in range(max_retries):
                try:
                    shutil.rmtree(jm_bot_downloads)
                    logger.info(f"已清理jm_bot下载目录: {jm_bot_downloads}")
                    break
                except Exception as e:
                    if attempt < max_retries - 1:
                        logger.warning(f"删除jm_bot下载目录失败，等待1秒后重试: {e}")
                        await asyncio.sleep(1)
                    else:
                        logger.error(f"删除jm_bot下载目录失败: {e}")

        logger.info(f"已清理用户 {user_id} 的所有相关文件")
    except Exception as e:
    ...
    try:
        ...
        if user_id in USER_COOLDOWNS:
            ...
            if remaining_time > 0:
                ...
                logger.info(f"用户 {user_id} 在CD中，剩余 {wait_time}")
                await send_group_message(group_id, f"您需要等待 {wait_time} 后才能再次下载。")
                return web.Response()
        
        await send_group_message(group_id, f"正在下载JM{jm_id}，请稍候...")
        
        try:
            if PDF_ENABLED:
                logger.info("PDF功能已启用，使用PDF下载方式")
                pdf_path = await download_pdf(jm_id, user_id)
                if not pdf_path or not os.path.exists(pdf_path):
                    logger.error("PDF下载失败")
                    await send_group_message(group_id, f"下载JM{jm_id}失败：PDF生成失败。")
                    return web.Response()
                ...
                if result:
                    ...
                    await send_group_message(group_id, f"JM{jm_id}下载完成！")
                ...
                logger.info(f"用户 {user_id} 进入CD，剩余 {COOLDOWN} 秒")
                return web.Response()
            else:
                logger.info("使用ZIP下载方式")
                user_download_dir = os.path.join(os.getcwd(), DOWNLOAD_DIR, str(user_id))
